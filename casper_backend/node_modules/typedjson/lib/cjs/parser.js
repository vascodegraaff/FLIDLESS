...
var TypedJSON = (function () {
    ...
    TypedJSON.prototype.parse = function (object) {
        ...
        var result;
        try {
            result = this.deserializer.convertSingleValue(json, type_descriptor_1.ensureTypeDescriptor(this.rootConstructor), this.getKnownTypes());
        }
        catch (e) {
            this.errorHandler(e);
        }
    ...
    TypedJSON.prototype.setSerializationStrategies = function (type, converters) {
        ...
        if (converters.deserializer != null) {
            this.deserializer.setDeserializationStrategy(type, function (value) { return converters.deserializer(value, {
                fallback: function (so, td) { return _this.deserializer.convertSingleValue(so, type_descriptor_1.ensureTypeDescriptor(td), _this.getKnownTypes()); },
            }); });
        }
        if (converters.serializer != null) {
            this.serializer.setSerializationStrategy(type, function (value) { return converters.serializer(value, {
                fallback: function (so, td) { return _this.serializer.convertSingleValue(so, type_descriptor_1.ensureTypeDescriptor(td)); },
            }); });
        }
    };
